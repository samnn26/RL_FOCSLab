...
def main():
    ...
    if continue_training:  # we need the DummyVecEnv to resume training, this is just an implementation issue
        env = DummyVecEnv([make_env()])  # unsure how this will work with multiprocesssing
        ...
        if number_of_cores == 1:
            ...
            env = SubprocVecEnv([make_env_multiproc('RWAFOCS-v22', i, env_args, log_dirs) for i in range(number_of_cores)])
        # env = Monitor(env, log_dir + 'training', info_keywords=('episode_service_blocking_rate','service_blocking_rate', 'throughput'))
        ...
        if number_of_cores == 1:  # unsure how to get plotter to work for vectorised envs
            results_plotter.plot_results([log_dir], 1e5, results_plotter.X_TIMESTEPS, "RWA")
            print("Whole training process statistics:") # can't work out how to use the get_attr() and env_method() methods of the SubprocVecEnv class
