...
class RWAEnvFOCSV4_82(OpticalNetworkEnv):
    ...
    def __init__(self, term_on_first_block, topology=None,
                 ...
                 seed=None, reset=True, exp_request_res=25e9, exp_request_lambda=1, capacity_scaling=1.0, norm_scaling=1.0,weight_service=0.99,weight_transmitter=0.01):
        ...
        self.normalised_links_lens = np.zeros(len(topology.edges()))
        self.weight_service = weight_service
        self.weight_availability = 0
    ...
    def initialise_availability(self):
        ...
        for idl1, l1 in enumerate(self.topology.edges()):
            prob_l = 0.75+self.normalised_links_lens[idl1]*0.25#
...
"""
def get_path_capacity(env: RWAEnvFOCSV4_82, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV4_82) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV4_82) -> Sequence[int]:
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV4_82) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV4_82):
        super().__init__(env)
