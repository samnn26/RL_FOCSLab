...
class RWAEnvFOCSV6(OpticalNetworkEnv):
    ...
    def __init__(self, topology=None,
        ...
        self.highest_weighted_utilisation = self.get_highest_weighted_utilisation()
        ...
        self.episode_actions_output = np.zeros((self.k_paths * self.num_spectrum_resources + self.reject_action),
                                               dtype=int)
        ...
        self.episode_actions_taken = np.zeros((self.k_paths * self.num_spectrum_resources + self.reject_action),
                                              dtype=int)
        """
        ...
        shape =  2 * self.topology.number_of_nodes() + self.topology.number_of_edges() #bit rate, src, dest, weighted_utilisation on edges
        self.initialise_weighted_utilisation()
        ...
        #edges = self.topology.number_of_edges() #to store the noise to signal ratio (weighted_utilisation) of the edges
    ...
    def initialise_lightpath_capacities(self):
        ...
        nch = 100  # call a method in gn model to retriev this
    ...
    def step(self, action: Sequence[int]):
        ...
        if path < self.k_paths and wavelength < self.num_spectrum_resources:  # if the indices are within the bounds
            ...
            self.service.destination][path], wavelength) >= self.service.bit_rate and self.snr_constraint_satisfied(path):
                ...
                self.update_weighted_utilisation(self.k_shortest_paths[self.service.source,self.service.destination][path])#increase weighted_utilisation when a new wavelength is been used

                self.service.accepted = True
                self.services_accepted += 1
                self.episode_services_accepted += 1
               # print("services accepted ", self.services_accepted, " and episode services accepted ", self.episode_services_accepted)
            ...
            self.service.destination][path], wavelength) >= self.service.bit_rate:
                ...
                self.episode_services_accepted += 1
                #print("services accepted ", self.services_accepted, " and episode services accepted ",
                      #self.episode_services_accepted)
    ...
    def reset(self, only_counters=False):
        ...
        self.topology.graph['available_wavelengths'] = np.zeros(
            (self.topology.number_of_edges(), self.num_spectrum_resources), dtype=int)
        self.no_valid_actions = 0
        self.lightpath_service_allocation = np.zeros([self.topology.number_of_nodes() *
                                                      (self.topology.number_of_nodes() - 1) * self.k_paths,
    ...
    def observation(self):
       ...
       # return [self.service.bit_rate,self.service.source_id,self.service.destination_id]
        ...
        weighted_utilisation_obs = np.full(shape=(1,self.topology.number_of_edges()), fill_value=-2.) #number of edges

        for id, lnk in enumerate(self.topology.edges()):
            link_weighted_utilisation = self.topology[lnk[0]][lnk[1]]['weighted_utilisation']
            #print("for link ", lnk[0], " and ", lnk[1], " weighted_utilisation is ", link_weighted_utilisation)
            weighted_utilisation_obs[:,id] = link_weighted_utilisation
        weighted_utilisation_obs = weighted_utilisation_obs/self.highest_weighted_utilisation

        #print(weighted_utilisation_obs) #weighted_utilisation is < 1 therefore stays within the bounds, no need to normalise

        return np.concatenate((source_destination_tau.reshape((1, np.prod(source_destination_tau.shape))),
                               weighted_utilisation_obs.reshape((1, np.prod(weighted_utilisation_obs.shape)))), axis=1)\
        ...
        for i in range(len(service.route.node_list) - 1):
            self.topology.graph['available_wavelengths'][
                self.topology[service.route.node_list[i]][service.route.node_list[i + 1]][
                    'index'], service.wavelength] -= 1
            try:
                ind_edge = self.topology[service.route.node_list[i]][service.route.node_list[i + 1]][
                    'running_services'].index(service.service_id)
                self.topology[service.route.node_list[i]][service.route.node_list[i + 1]]['running_services'].remove(
                    service.service_id)
                del self.topology[service.route.node_list[i]][service.route.node_list[i + 1]][
                    'running_service_wavelengths'][ind_edge]
            except:
    ...
    def _release_path(self, service: Service):
        ...
        if service.new_lp:
            ...
            self.episode_num_transmitters[int(service.source) - 1] -= 1
            self.episode_num_receivers[int(service.destination) - 1] -= 1
        self.update_weighted_utilisation(service.route)
    ...
    def initialise_weighted_utilisation(self):
        for id, lnk in enumerate(self.topology.edges()):
            link_length = self.topology[lnk[0]][lnk[1]]['length']
            wavelen_weighted_utilisation = link_length*1 # call per link
            self.topology[lnk[0]][lnk[1]]['weighted_utilisation'] = wavelen_weighted_utilisation#weighted_utilisation if all the lightpaths are switched on one channel

    def get_highest_weighted_utilisation(self):
        weighted_utilisations = []
        for id, lnk in enumerate(self.topology.edges()):
            link_length = self.topology[lnk[0]][lnk[1]]['length']
            weighted_utilisations.append(link_length* self.num_spectrum_resources) # call per link
        return max(weighted_utilisations)

    def update_weighted_utilisation(self,path):
        for i in range(len(path.node_list) - 1):
            link_length = self.topology[path.node_list[i]][path.node_list[i + 1]]['length']
            num_wavelengths = np.sum(self.topology.graph['available_wavelengths'][self.topology[path.node_list[i]][path.node_list[i + 1]]['index'],:])
            new_weighted_utilisation = link_length* num_wavelengths# call per link
            self.topology[path.node_list[i]][path.node_list[i + 1]]['weighted_utilisation'] = new_weighted_utilisation
...
"""
def get_path_capacity(env: RWAEnvFOCSV6, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV6) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_first_fit(env: RWAEnvFOCSV6) -> Sequence[int]:
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV6) -> Sequence[int]:
    best_hops = np.finfo(0.0).max  # in this case, shortest means least hops
    decision = (env.k_paths, env.num_spectrum_resources)  # stores current decision, initilized as "reject"
    for idp, path in enumerate(env.topology.graph['ksp'][env.service.source, env.service.destination]):
        if path.hops < best_hops:  # if path is shorter
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV6) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV6):
        super().__init__(env)
