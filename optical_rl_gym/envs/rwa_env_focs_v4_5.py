...
class RWAEnvFOCSV4_5(OpticalNetworkEnv):
...
"""
def get_path_capacity(env: RWAEnvFOCSV4_5, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV4_5) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV4_5) -> Sequence[int]:
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV4_5) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV4_5):
        super().__init__(env)
