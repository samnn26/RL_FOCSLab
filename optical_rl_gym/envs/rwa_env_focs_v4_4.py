...
class RWAEnvFOCSV4_4(OpticalNetworkEnv):
    ...
    def _update_link_stats(self, node1, node2):
        ...
        if self.current_time > 0:
            ...
            """
            # self.topology[node1][node2]['utilization'] = np.sum(self.topology.graph['available_wavelengths'][self.topology[node1][node2]['index']])
            self.topology[node1][node2]['utilization'] = np.count_nonzero(self.topology.graph['available_wavelengths'][self.topology[node1][node2]['index']])
...
"""
def get_path_capacity(env: RWAEnvFOCSV4_4, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV4_4) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV4_4) -> Sequence[int]:
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV4_4) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV4_4):
        super().__init__(env)
