...
class RWAEnvFOCSV4_7(OpticalNetworkEnv):
    ...
    def reward(self, path):  # overwrite reward function
        ...
        # breakpoint()
        alpha = 0.1 # scale down the values a bit, maximum is 2165 for load term and 81 for
        beta = 0.1
        return alpha*(1/path_load) + beta*(self.max_path_length/path.length)
...
"""
def get_path_capacity(env: RWAEnvFOCSV4_7, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV4_7) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV4_7) -> Sequence[int]:
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV4_7) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV4_7):
        super().__init__(env)
