...
class RWAEnvFOCSV5(OpticalNetworkEnv):
    ...
    def initialise_nsr(self):
        for id, lnk in enumerate(self.topology.edges()):
            ...
            wavelen_nsr = GN_model_multiband.calculate_per_channel_nsr_for_link(link_length, 1)  # call per link
    ...
    def increase_nsr(self,path,wavelength):
        for i in range(len(path.node_list) - 1):
            ...
            if wavelength>50:
                wavelen_nsr = GN_model_multiband.calculate_per_channel_nsr_for_link_lband(link_length, wavelength)# call per link
            else:
                wavelen_nsr = GN_model_multiband.calculate_per_channel_nsr_for_link(link_length,wavelength)  # call per link
    ...
    def decrease_nsr(self,path,wavelength):
        for i in range(len(path.node_list) - 1):
            link_length = self.topology[path.node_list[i]][path.node_list[i + 1]]['length']
            if wavelength>50:
                wavelen_nsr = GN_model_multiband.calculate_per_channel_nsr_for_link_lband(link_length,
                                                                                wavelength)  # call per link
            else:
                wavelen_nsr = GN_model_multiband.calculate_per_channel_nsr_for_link(link_length, wavelength)  # call per link
            link_nsr = self.topology[path.node_list[i]][path.node_list[i + 1]]['nsr']
            #print("for link ",self.topology[path.node_list[i]][path.node_list[i + 1]]['id'], " nsr is ", link_nsr)
...
"""
def get_path_capacity(env: RWAEnvFOCSV5, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV5) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_first_fit(env: RWAEnvFOCSV5) -> Sequence[int]:
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV5) -> Sequence[int]:
    best_hops = np.finfo(0.0).max  # in this case, shortest means least hops
    decision = (env.k_paths, env.num_spectrum_resources)  # stores current decision, initilized as "reject"
    for idp, path in enumerate(env.topology.graph['ksp'][env.service.source, env.service.destination]):
        if path.hops < best_hops:  # if path is shorter
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV5) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV5):
        super().__init__(env)
