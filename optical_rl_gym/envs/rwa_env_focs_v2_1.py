...
class RWAEnvFOCSV2_1(OpticalNetworkEnv):
...
#p = self.k_shortest_paths[source, dest][path_ind]
        ...
        if path < self.k_paths and wavelength < self.num_spectrum_resources:  # if the indices are within the bounds
            ...
            self.service.destination][path], wavelength) >= self.service.bit_rate:  # if path is free and has sufficient capacity
                ...
                self.update_nsr(self.k_shortest_paths[self.service.source, self.service.destination][
                                    path])  # increase nsr when a new wavelength is been used
...
"""
def get_path_capacity(env: RWAEnvFOCSV2_1, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV2_1) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV2_1) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV2_1):
        super().__init__(env)
