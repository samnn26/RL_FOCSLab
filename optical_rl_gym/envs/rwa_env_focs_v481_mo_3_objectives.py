...
class RWAEnvFOCSV4_81(OpticalNetworkEnv):
    ...
    def __init__(self, term_on_first_block, topology=None,
                 ...
                 seed=None, reset=True, exp_request_res=25e9, exp_request_lambda=1, capacity_scaling=1.0, norm_scaling=1.0,weight_service=0.9,weight_failure_score=0.1,weight_transmitter=0.0):
        ...
        self.episode_per_link_services = np.zeros(self.topology.number_of_edges())
        self.link_lens = np.zeros(len(topology.edges()))
        self.unit_failure_probability = 0.00000001
        self.weight_service = weight_service
        self.weight_availability = weight_failure_score
        self.weight_transmitter = weight_transmitter
        self.episode_failure_score = 0.0
        ...
        self.calculate_normalise_link_length_based_availabiliity()
        self.initialise_availability()
    ...
    def calculate_normalise_link_length_based_availabiliity(self):
        max_len =0
        links_lens = np.zeros(len(self.topology.edges()))
        for id, lnk in enumerate(self.topology.edges()):
            link_length = self.topology[lnk[0]][lnk[1]]['length']
            if link_length > max_len:
                max_len = link_length
            links_lens[id] = link_length
        for i in range(0,len(self.topology.edges())):
            self.link_lens[i] = links_lens[i]#/max_len changed this
    def initialise_availability(self):
        #initialise node and link availability from uniform distribution
        #based on joint probability of respective nodes and links calculate path availability

        totalLinks = len(self.topology.edges())
        node_availability = []
        link_availability = []
        # for idn1, n1 in enumerate(self.topology.nodes()):
        #     prob_n = np.random.uniform(0.75,1.0)
        #     node_availability.append(prob_n)
        for idl1, l1 in enumerate(self.topology.edges()):
            #prob_l = 0.75+self.links_lens[idl1]*0.25#
            prob_l = 1 - self.link_lens[idl1]*self.unit_failure_probability #(self.links_lens[idl1] * 0.25)
            link_availability.append(prob_l)
            print("availability of link ", idl1, " is ", prob_l )
            ...
            for idn2, n2 in enumerate(self.topology.nodes()):
                ...
                if idn1 != idn2:
                    for p in range(self.k_paths):
                        path = self.k_shortest_paths[n1, n2][p]
                        path_availability = 1.0

                        for i in range(len(path.node_list) - 1):
                            link_id = self.topology[path.node_list[i]][path.node_list[i + 1]]['index']
                            link_prob = link_availability[link_id-1]
                            path_availability = path_availability *link_prob

                        #     node = int(path.node_list[i])
                        #     print("node id:",node)
                        #     node_prob = node_availability[node-1]
                        #     path_availability = path_availability*node_prob
                        #
                        # last_node_index = len(path.node_list)-1
                        # last_node = int(path.node_list[last_node_index])
                        # node_prob = node_availability[last_node-1]
                        # path_availability = path_availability*node_prob

                        path.availability = path_availability
                        print("path availability ", path_availability)
    ...
    def reward(self):  # overwrite reward function
        ...
        if self.service.accepted:
            reward = 1 * self.weight_service
            path = self.service.route
            path_availability = path.availability
            avaialability_score = path_availability * self.weight_availability
            reward = reward + avaialability_score
            if self.service.new_lp == True:
                reward = reward - 1 * self.weight_transmitter
                path = self.service.route

            self.episode_failure_score = self.episode_failure_score + (1 - path_availability)


        if reward < 0:
            reward = 0
        return reward
    # def reward(self, path):  # overwrite reward function
    #
    #     if not self.service.accepted:
    #         return 0
    #     norm_ts = ((self.max_cap*1e12)/(self.exp_request_res))*self.num_spectrum_resources
    #     path_load = 0
    #     for i in range(len(path.node_list)-1):
    #         path_load += (np.sum(self.topology.graph['available_wavelengths'][
    #                   self.topology[path.node_list[i]][path.node_list[i + 1]]['index'],:])) / norm_ts
    #
    #     return 0.1*(1/path_load)
    ...
    def get_max_capacity(self):
        ...
        self.max_path_length = max(lengths)
        return np.max(arr)
    ...
    def step(self, action: int):
        ...
        reward = self.reward()
        #reward = self.reward(self.k_shortest_paths[self.service.source, self.service.destination][path])
        ...
        info = {
            ...
            'transmitters' : self.num_transmitters,
            'failure_score' : self.episode_failure_score,
            'per_link_service_distributioin' : self.episode_per_link_services.flatten().tolist()
        }
    ...
    def observation(self):
        ...
        return {'utilisation_ts':  utilisation_ts,
    ...
    def _provision_path(self, path: Path, wavelength: int):
        ...
        for i in range(len(path.node_list) - 1):
            ...
            ind_edge = self.topology[path.node_list[i]][path.node_list[i + 1]]['index']
            service_count = self.episode_per_link_services[ind_edge]
            service_count = service_count - 1
            self.episode_per_link_services[ind_edge] = np.count_nonzero(
            self.topology[path.node_list[i]][path.node_list[i + 1]]['running_services'])  # service_count
...
"""
def get_path_capacity(env: RWAEnvFOCSV4_81, path: Path) -> int:
    capacity = 0
...
"""
def shortest_path_first_fit(env: RWAEnvFOCSV4_81) -> Sequence[int]:
    for wavelength in range(env.num_spectrum_resources):
...
"""
def shortest_available_path_last_fit(env: RWAEnvFOCSV4_81) -> Sequence[int]:
...
def least_loaded_path_first_fit(env: RWAEnvFOCSV4_81) -> Sequence[int]:
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    ...
    def __init__(self, env: RWAEnvFOCSV4_81):
        super().__init__(env)
