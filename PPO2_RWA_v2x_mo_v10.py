...
parser.add_argument('--weight_service', default='0.9', type=float)
parser.add_argument('--weight_failure_score', default='0.1', type=float)
parser.add_argument('--weight_transmitter', default='0.1', type=float)
...
weight_failure_score = args.weight_failure_score
weight_transmitter = args.weight_transmitter
...
def make_env(env_args, log_dir):
    def maker():
        ...
        'episode_services_processed', 'services_accepted', 'services_processed', 'throughput','failure_score','per_link_service_distributioin','transmitters'))
        return env
    return maker
...
def main():
    ...
    #with open('./topologies/nsfnet_chen_5-paths_rounded.h5', 'rb') as f:
    with open('./topologies/gb_5-paths.h5', 'rb') as f:
   ...
   #     topology = pickle.load(f)
    ...
    # with open(f'/Users/joshnevin/RL_FOCSLab/topologies/nsfnet_chen_5-paths_directional.h5', 'rb') as f:
        topology = pickle.load(f)
    node_request_probabilities = np.array([0.083, 0.083,0.083,0.083,0.083, 0.083,0.083,0.083,0.083, 0.083,0.083,0.083])
    ...
    env_args = dict(topology=topology, seed=10, load = load,
                    ...
                    exp_request_res=requestsize, term_on_first_block=termfirstblock, num_spectrum_resources=100,weight_service=weight_service,weight_failure_score=weight_failure_score,weight_transmitter=weight_transmitter)
    ...
    if len(continuedexp)>0:
        ...
        if number_of_cores == 1:
            log_dir = "./tmp/RWAFOCS-ppo/gb/"+today+exp_num+"_"+str(weight_service)+"/"
           ...
           # else:
            #agent = MaskablePPO('MlpPolicy', env, verbose=0, policy_kwargs=policy_args, gamma=gamma, learning_rate=learningrate, batch_size = batchsize, device='cuda')
            agent = PPO('MlpPolicy', env, verbose=0, policy_kwargs=policy_args, gamma=gamma,
                                learning_rate=learningrate, batch_size=batchsize, device='cuda')
