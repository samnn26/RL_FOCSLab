...
def main():
    ...
    env_args = dict(topology=topology, seed=10, load = load,
                    ...
                    exp_request_res=requestsize, exp_request_lambda=1, term_on_first_block=termfirstblock, num_spectrum_resources=100,capacity_scaling=sf)
    ...
    if len(continuedexp)>0:
        ...
        for i in range(number_of_cores):
            log_dirs.append("./tmp/RWA/"+today+exp_num+"/_core_"+str(i)+"/")
            os.makedirs("./tmp/RWA/"+today+exp_num+"/_core_"+str(i)+"/", exist_ok=True)
        ...
        model_dir = "./tmp/RWA/"+continuedexp+"/_core_0"
        ...
        if number_of_cores == 1:
            ...
            if gpu:
                agent = MaskablePPO('MultiInputPolicy', env, verbose=0, policy_kwargs=policy_args, gamma=.95, learning_rate=10e-5, device='cuda')
            ...
            if gpu:
                agent = MaskablePPO('MultiInputPolicy', env, verbose=0, policy_kwargs=policy_args, gamma=gamma, learning_rate=learningrate, batch_size = batchsize, device='cuda')
            else:
                agent = MaskablePPO('MultiInputPolicy', env, verbose=0, policy_kwargs=policy_args, gamma=gamma, learning_rate=learningrate, batch_size = batchsize, device = 'cpu')
