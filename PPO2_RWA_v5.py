...
env_args = dict(topology=topology, seed=10,
                ...
                episode_length=3000, node_request_probabilities=node_request_probabilities,term_on_first_block=False)
# breakpoint()
# Create log dir
log_dir = "./tmp/RWAFOCS-ppo_v5/"
os.makedirs(log_dir, exist_ok=True)
callback = SaveOnBestTrainingRewardCallback(check_freq=1000, log_dir=log_dir)

#env = gym.make('DeepRMSA-v0', **env_args)
env = gym.make('RWAFOCS-v5', **env_args)
...
agent = MaskablePPO('MlpPolicy', env, verbose=0, tensorboard_log="./tb/PPO-RWA-v5/", policy_kwargs=policy_args, gamma=.95, learning_rate=10e-5)


#a = agent.learn(total_timesteps=10000, callback=callback)
a = agent.learn(total_timesteps=9000000, callback=callback)
results_plotter.plot_results([log_dir], 1e7, results_plotter.X_TIMESTEPS, "RWA_V5")
